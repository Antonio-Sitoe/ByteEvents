...
import type { ICreateEventData, IEventData } from '../schemas/events'

class EventModel {
  async create(eventData: ICreateEventData): Promise<IEventData | undefined> {
    const { title, description, start_datetime, end_datetime, location, organizer_id, status } =
      eventData
  ...
  async findAll(): Promise<IEventData[]> {
    const results = await db.select().from(events).orderBy(desc(events.start_datetime))

    return results
  }

  async update(id: string, eventData: Partial<IEventData>): Promise<IEventData | undefined> {
    const { title, description, start_datetime, end_datetime, location, status } = eventData
    ...
    if (start_datetime != null) changes.start_datetime = new Date(start_datetime)
    if (end_datetime != null) changes.end_datetime = new Date(end_datetime)

    if (status != null) {
      if (status === 'PUBLISHED' || status === 'DRAFT' || status === 'FINISHED') {
